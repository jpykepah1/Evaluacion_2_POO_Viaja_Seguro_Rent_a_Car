...
### Added
- ES3/UF support for arriendos: new fields `valor_uf_fecha` and `fecha_uf_consulta` in `modelo/arriendo.py` and persisted in DB (`create_updated.sql`).
- Controller integrates with `IndicadorService.obtener_uf_por_fecha()` and calculates CLP cost from UF.
- New DAO methods `listarArriendosConRelacion` and `listarArriendosPorFechaConRelacion` to fetch arriendos with related vehicle/client fields in a single query.
- Security helpers added to `MVC/validador_formatos.py` (SQL detection, sanitiser, strong password validator, cipher wrapper).

### Changed
- Removed presentation attributes from models; presentation assembled in DTO/controller.
- DAO robustness: replaced `SELECT *` with explicit column lists and added proper numeric casting to avoid conversion errors.
- `vehiculo.precio_diario` semantics: treated as UF units in application logic; sample data updated accordingly.

### Fixed
- Multiple column-mapping bugs in DAOs fixed, preventing string->float conversion errors.

### Notes
- Use parameterized SQL queries in DAOs; the new helpers add detection but are not a substitute for prepared statements.
